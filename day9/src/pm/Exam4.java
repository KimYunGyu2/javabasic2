...
/*final...필드, 메서드, 클래스 앞에 위치한다. 의막 각각 다르다.
final 변수 : 변수를 선언할때만 지정할 수 있고 한번 대입된 값을 수정할 수 없다.
 처음 대입한 값이 최종값이 된다. 변수를 상수로 사용할 때 쓴다.
 final은 일단 한번 값이 입력되고 나면 절대로 값을 수정할수 없다.
 생성자라도 더이상은 변경이 불가능하다.
 		...
 		final double c = 3.14;
 		// 어떤 필요에 따라서 복사본을 하나 만들어 원본이 삭제된 후에도 그 값을 활요할수
 		// 있도록 하는것이 final의 기능이다.
*/
