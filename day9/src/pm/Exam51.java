...
class Animal {
 ...
 }						      추상 매서드를 포함하는 클래스는 반드시 추상 클래스로 정의해야한다.
 			                            추상 클래스도 클래스 이므로 상속도 할수 있다.
                                                   추상 클래스로는 객체를 생성할 수는 없다.
...
animal2.cry(); // 멍멍
 ...
 다형적 표현을 사용할때도 cry() 매서드를 홏출한다.
 
 추상 매서드 사용의 장점
 
 일반 클래스로 정의 했을때와 추상 클래스로 정의 했을때
 ...
 cry() 매서드를 호출해도 아무것도 출력되지 않는다. 부모의 cry() 매서드에는 아무것도 정의되지 않았기
 때문이다.
 추상 클래스를 생각해보면...
 자식 클래스가 오버라이딩 하려고 하는 과정에서 오타가 발생하면 문법오류가 발생한다.
추상 클래스를 상속받으면 추상 클래스에 정의해 놓은 추상 매서드들을
구체화한 클래스를 만들어야만 한다. 매서드 오버라이딩을 강제하는 방식으로 생각하자

겨우 오타 찾는 정도의 장점이라고 가볍게 볼수도 있겠지만 여러 사람들이나 여러회사들이 협업을
하게 되는 경우 많은 필요성을 느끼게 될것이다.

정리...
만약 abc()라는 ㅊ추상 매서드를 포함하고 있는 추상 클래스가 있을때 '이것을 상속한
모든 자식 클래스들 내부에는 항상 반드시 abc() 매서드가 정의되 있어야만 한다.' 라는 사실을
보장하게 되는 것이다.
