...
1. 클래스의 개념
	...
	클래스 : 구죠체는 데이터만 관리할 수 없다. 여기에 어떤 기능들(매서드)을
	    추가한 것이 바로 클래스이다.
	  ...
	  클래스는 다시 일반클래스와 추상 클래스로 나눌수있다.
	  
	  크게보면 2개(클래스와 인터페이스)
	  작게 보면 3개 일반클래스, 추상클래스, 인터페이스 의 객체 지향 문법 요소를 사용한다.
	  
	  클래스명과 파일명은 반드시 일치해야만 한다.
	  클래스의 밖에 올수 있는 것은 package import 클래스 선언
	  
	  접근 지정자 public 은 한개의  java 파이 안에서 최대 1개의 클래스에만 붙일 수 있다.
	  
	  객체와 인스턴스
	  객체라는 표현 인스턴스 라는 표현은 같은 대상을 가리킨다.
	  클래스와의 관계를 표현할때는 인스턴스라고 하고
	  단독으로 그 자체만 표현할때는 객체라고 한다.
	  예) 인스턴스는 붕어빵 기계로 찍어낸 붕어빵
	         객체는 그냥 붕어빵
