...
try{
 ...
 최초 try 구문이 실행되어 만약 예외가 발생하지 않으면 catch는 실행되지 않는다. 그리고 finally 블록 구문을 실행한다.
 그런데 try 구문이 실행되어 만약 예외가 발생하면 자바가 먼저 인지하여 예외 타입의 객체를 생성하여 catch의 매개변수로 전달한다.
 이것은 마치 자바가 catch() 라는 이름의 매서드를 호출하는 것과 비슷하다. 만약 자바가 생성해서 넘겨준 객체타입을 catch블록이 처리
 할수 없을때 즉 해당 객체를 받을 catch() 블록이 존재하지 않으면 예외 처리가 되지 않는다.
...
//		이것은 또 마치 if - else if - else 구문에서 조건식을 위에서부터 검사하는 것과 같다.
		
		//1개의 catch() 블록으로 2개의 예외를 동시에 처리하도록 통합해보자. 둘중 어떤 예외가 발생해도 모두
		//하나의 예외처리로 해결할수 있다.
		...
		//예를 들면 파일을 열어서 사용한뒤에 닫아야 다른 프로그램이 이 파일을 사용할 수 있다.
