...
일반에외checked exception 클래스와 실행예외unchecked exception
클래스로 나뉜다.
...
반면 실행 예외는 컴파일 전이 아니라 실행할 때 발생하는 예외로 예외 처리를 하지 않더라도
실행은 된다. 따라서 프로그램 실행 중에 프로그램이 강제 종료 되ㅣ는 이유는 대부분
실행 예외 때문이다.
...
1. 일반예외 일반 예외는 예외 처리를 하지 않으면 컴파일 자체가 불가능해진다.
   ...
   해당 쓰레드를 일시 정지 상태로 만드는 Thread 클래스의 매서드이다.
   이 매서드는 일반 예외가 발생할 수 있다. 때문에 반드시 예외처리를 해야한다.
   예외처리를 생략하면 문법 오류가 발생해서 컴파일 자체가 불가능.
...
분모가 0일때 즉 0으로 나눌때 이다. 이렇게 계산을 하게 되면 예외가 발생된다.
...
/*
 ...
 '가능할수도','불가능할수도' 있다.
 clasCastException 다운 캐스팅이 불가능한 상황에서 다운캐스팅을 시도할 때 발생된다.
