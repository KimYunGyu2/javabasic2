...
출력되는 결과 즉 리턴 값의 자료형은 int 이고 접근은 public 으로 선언되어있다.


매서드 호출
매서드도 클래스의 멤버이므로 객체안에 존재한다 클래스 외부에서 매서드를 사용하려면 객체를 먼저
생성해야 한다.
클래스 내부에 있는 매서드 끼리는 객체를 생성하지 않고 서로를 호출할수 있다.

매서도 오버로딩

매개변수의 갯수와 자료형에 따라서각기 다른 매서드가 실행되는 것을 의미한다.
print();
print(3);
print(5.89);
print(2, 4);

4개의 print()매서드가 오버로딩, 각각은 다른 매개변수의 개수 또는 타입에 따라
다른 각각의 매서드가 실행된다.
