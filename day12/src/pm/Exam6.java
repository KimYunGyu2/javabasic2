...
public class Exam6 extends Thread{
	...
	2가지 방법
	1. 쓰레드 클래스를 상속받아 run() 매서드를 오버라이딩 하는 방법
	...
	한번 실행된 쓰레드 객체는 재사용할 수 없다는 것이다. 만약 다시 실행하려면 객체를
	다시 생성해야만 한다

1. 쓰레드 클래스를 상속 받아서 run()매서드를 오버라이딩
	step 1 클래스 정의
	...
	*/
	...
	public static void main(String[] args) {
		...
		smiFileThread.start();  //객체를 생성하여 start()매서드로 실행하였다.
...
/*
 ...
 적어도 하나 이상의 쓰레드를 추가로 생성하여 실행해야 한다.
  ...
  참고로 멀티 쓰레드는 독립적으로 실행되기 때문에 먼저 start()매서드로 호출이 되었다 하더라도
  나중에 실행된 쓰레드 보다 늦게 실행될수 있다.
