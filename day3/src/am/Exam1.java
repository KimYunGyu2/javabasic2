...
/*
 ...
 중괄호 안의 실행 구문을 반복적으로 실행하는 반복 제어문 조건을 만족하는 동안
 반복은 지속된다.
  while (조건) {
             실행구문;
  }
...
//		} while(a < 0);            // 1회가 반드시 실행
		...
		// 초기값이 a = 0 이므로 처음부터 조건식 a < 0 은 false이다.
		// 따라서 while문으로 진입할 수 없으므로 단 한 번도 실행되지 않는다.
		// 반면 do~while문일때는 일단 한번은 실행한 후에 조건식을
...
//		}while(a < 10);
		...
		// 1회 이상 반복될때는 동일한 결과가 나타난다. 즉 , while문이
		// 0번 반복할때만 차이가 있다
