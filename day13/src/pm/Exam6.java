...
/*
 ...
 이 제네릭 타입은 필드 타입 getter매서드의 리턴 타입 setter매서드의 입력타입에
 사용됐다.
 
 제네릭 인터페이스인 MyInterface<K,V> 2개의 제네릭 타입변수를 갖고 있고
 각각 setter와  getter 매서드의 입력과 리턴타입으로 사용되고 있다.
 ...
 */
	public static void main(String[] args) {
		 ...
		 /*
		  ...
		  하지만 모든 객체는 Object타입이기 때문에 실수로 엉뚱한 객체를 저장하더라도
		  오류를 발생하지는 않는다. getter 매서드로 꺼내온 값도 Object 타입이므로
		  항상 다운캐스팅을 수행해야만 했다.
		  하지만 제네릭 클래스를 사용한 mc1, mc2 객체 생성 예를 보면 입출력이 각각 객체를
		  생성할때 정해진 타입으로 확정되기 때문에 setter 매서드를 사용할때
		  입력 타입을 정확히 확인할 수도 있고 출력도 해당 타입으로 리턴이 되기때문에
		  다운 캐스팅 자체가 필요없어진다.
		  */
