...
// 패키지를 생성하면 클래스를만들때 첫번째 줄에 반드시 'package 패키지명;'이
// 명시되어야만 한다.
		
// 패키지 사용의 장점은 패키지의 영향으로 클래스가 저장되는 공간이 분리되어
// 클래스명의 충돌을 방지 할 수 있다는 점이다.
		
// 임포트는 다른 패키지 내의 클래스를 사용하기 위한 목적인 문법이다.
// 패키지 선언 ㄷ4ㅏ음 줄에 위치한다. 일반적으로 자바가 동작할 때는 자신이 속한 패키지 내부에
// 위치한 클래스만을 사용할수 있다.
...
// 여러개 클래스를 임포트
//		임포트는 마치 자신의 내부에 있는 것처럼 사용하는 것이다.
//		사실 실제 임포트 되는 대상은 .java가 아니라 컴파일이 완료된 .class 이다.
		
// 여러개 클래스를 임포트 *을 사요ㅕㅇ한다.
// import am.A;
// import am.B;
// import am.C;
// --------> import am.*;
